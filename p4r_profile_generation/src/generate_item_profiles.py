...
# BUSINESS = ITEM = ENTITY: In the context of this prompt, 'business' refers to 
# any entity (e.g., restaurant, store, service) that is being analyzed as a discrete item.
PROMPT_BUSINESS_PROFILE = (
...
def process_business_batch(batch_data):
    ...
    try:
        ...
        prompt = PROMPT_BUSINESS_PROFILE.replace("<businesses>", businesses_str)
...
if __name__ == "__main__":
    ...
    business_df = pd.read_csv(r"business_df.csv")
    ...
    df_final.to_csv("p4r_profile_generation/outputs/business_profiles.csv", index=False)
    print("Business profiles saved to: p4r_profile_generation/outputs/business_profiles.csv")
    ...
    with open("p4r_profile_generation/outputs/business_profiles.json", "w", encoding="utf-8") as f:
        json.dump(json_data, f, indent=2, ensure_ascii=False)
    print("JSON profiles saved to: p4r_profile_generation/outputs/business_profiles.json")
